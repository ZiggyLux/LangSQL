...
function random_ids ($cIds) {
    ...
    $result = exec_query("SELECT max(id) FROM ruvoc");
	    ...
	    for ($i=0; $i<$cIds; $i++) {
	    	while (TRUE) {
				...
				/* Vérification	existence dans BD */
		   	    $result_x = exec_query("SELECT id FROM ruvoc WHERE id={$x}");
    ...
    if ($cQuestion > 0) {
	    ...
	    for ($i=0; $i<$cQuestion; $i++) {
	    	while (TRUE) {
				...
				/* Vérification	existence dans BD */
		   	    $result_x = exec_query("SELECT id FROM ruvoc WHERE id={$idItem}");
				if (!mysql_fetch_array($result_x, MYSQL_ASSOC)) {
				    mysql_free_result($result_x);
					continue; /* Inexistent dans BD, on recommence ce tirage */
				}
